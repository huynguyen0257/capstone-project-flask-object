...
def get_image_info(image, bboxes, classes=read_class_names(cfg.YOLO.CLASSES), show_label=True):
    num_classes = len(classes)
    image_h, image_w, _ = image.shape
    hsv_tuples = [(1.0 * x / num_classes, 1., 1.) for x in range(num_classes)]
    ...
    for i in range(num_boxes[0]):
        ...
        result = {
            ...
            "bottom": int(coor[2]),
            "left": int(coor[1]),
            "code": classes[class_ind]
        }
        arr_object.append(classes[class_ind])
        arr_result.append(result)
    shape = {"height": image_h, "width": image_w}
    result = {"shape": shape, "info": arr_result, "objects": arr_object}
    return result
    
def get_image_info_mask(image, bboxes, classes=read_class_names(cfg2.YOLO.CLASSES), show_label=True):
    num_classes = len(classes)
    image_h, image_w, _ = image.shape
    hsv_tuples = [(1.0 * x / num_classes, 1., 1.) for x in range(num_classes)]
